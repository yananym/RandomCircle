function generateCircles() {
	...
	for (var i = 0; i < 100; i += 1) {
	  ...
	  html += '<div id="circle'
		  ...
		  + delay + 'ms;"><span>'
		  + names[name] + '</span><span class="removeCircle ms-fontWeight-semilight" title="Delete Planet"><i class="ms-Icon ms-Icon--x"></i></span></div>';
	}
...
function resizeCircles() {
	...
	valueSize.max = "200";
	var valueNumber = document.getElementById("circleNumber");
	valueNumber.max = "200";
	var displaySize = document.getElementById("circleSizeTitle").lastChild;
	displaySize.innerHTML = valueSize.value;
	var displayNumber = document.getElementById("circleNumberTitle").lastChild;
	displayNumber.innerHTML = valueNumber.value;

	valueSize.addEventListener("change", function (event) {
	var elements = document.querySelectorAll(".circle");
		for (var i = 0; i < elements.length; i++) {
			var el = document.getElementById("circle" + i);
			var width = parseInt(el.style.width) * valueSize.value / 100;
			var height = parseInt(el.style.height) * valueSize.value / 100;
